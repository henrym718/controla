...
import ConteoClient, { type ConteoEstado } from "./conteo-client";
...
export default async function ConteoPage({
  ...
  const { data } = await db.rpc("conteo_estado", {
    p_restaurant: session.restaurant_id,
    p_date: today,
  });

  return <ConteoClient today={today} initial={data as unknown as ConteoEstado} />;
}
